...
public class GraphTest3{
	public static void main(String[] args){
		...
		double[][] x = UtilMethods.concat(UtilMethods.concat(UtilMethods.standardDist(0, 0, 0.1, 100), UtilMethods.standardDist(0, 0.5, 0.1, 100)), UtilMethods.concat(UtilMethods.standardDist(0.5, 0, 0.1, 100), UtilMethods.standardDist(0.5, 0.5, 0.1, 100)));
		...
		for(int i = 0; i < y3.length; i++){
			y3[i] = UtilMethods.oneHotEncode(1, 4);
		}
		double[][] y4 = new double[100][4];
		for(int i = 0; i < y4.length; i++){
			y4[i] = UtilMethods.oneHotEncode(0, 4);
		}
		double[][] y = UtilMethods.concat(UtilMethods.concat(y1, y2), UtilMethods.concat(y3, y4));
		net.fit(x, y, 1000, 1, Loss.crossEntropy, new AdamOptimizer(0.01), 0, true, false);
